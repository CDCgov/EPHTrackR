...
#' 
#' 
# list_StratificationTypes(measure=370,format="ID")

# list_StratificationTypes(measure=c(370,423,707),format="ID")
...
list_StratificationTypes <-
  function(measure=NA,
    ...
    GL_list<-list_GeographicTypes(measure,format)
    ...
    MS_list<-purrr::map(1:length(meas_ID),function(measstrat){
      ...
      MS<-
        httr::GET(paste0("https://ephtracking.cdc.gov:443/apigateway/api/v1/stratificationTypes/",
                         meas_ID[measstrat],"/",
